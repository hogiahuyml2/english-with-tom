...
// Helper: merge teacher edits (comment + visibility + error_list đã lọc) vào feedback JSON
function mergeTeacherEdits(existingFeedback, { teacher_comment, visibility, error_list } = {}) {
  let fb = {};
...
app.post('/api/teacher/save-draft/:id', requireRole('teacher','admin'), (req, res) => {
  const subId = Number(req.params.id);
  const { score, max_score, teacher_comment, visibility, error_list } = req.body || {};
  const sub = db.prepare('SELECT id, feedback FROM submissions WHERE id=?').get(subId);
  if (!sub) return res.status(404).json({ error: 'Không tìm thấy.' });
  const feedbackJson = mergeTeacherEdits(sub.feedback, { teacher_comment, visibility, error_list });
