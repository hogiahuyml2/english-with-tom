...
overall_score = SUM 3 tiêu chí (Content + Organisation + Language), KHÔNG tính trung bình — cộng thẳng, thang 0–15. scale_label = "A2 Key (0–15)".
criteria gồm ĐÚNG 3 mục: "Content (Nội dung)", "Organisation (Tổ chức)", "Language (Ngôn ngữ)". Mỗi mục có max=5.
Ví dụ: Content=4, Organisation=3, Language=4 → overall_score=11 (không phải 3.67).
...
overall_score = SUM 4 tiêu chí (Content + Comm.Ach. + Organisation + Language), KHÔNG tính trung bình — cộng thẳng, thang 0–20. scale_label = "B2 First (0–20)".
Ví dụ: Content=4, Comm.Ach.=3, Organisation=4, Language=4 → overall_score=15 (không phải 3.75).
...
overall_score = SUM 4 tiêu chí (Content + Comm.Ach. + Organisation + Language), KHÔNG tính trung bình — cộng thẳng, thang 0–20. scale_label = "B1 Preliminary (0–20)".
...
function rubricFor(exercise) {
  ...
  // Giáo viên chọn part/task cụ thể qua _task_override; dùng làm title hint
  const titleHint = exercise._task_override || exercise.title || '';
  if (program === 'KET') return ketRubric(titleHint);
  if (program === 'FCE') return fceRubric(titleHint);
  if (program === 'PET') return petRubric(titleHint);
  if (program === 'IELTS') return ieltsRubric(titleHint);
  if (program === 'APTIS') {
    // Part 1 — Word-level (scale 0–3, chỉ 1 tiêu chí)
    if (/component\s*1|part\s*1/i.test(titleHint)) {
...
suggested_writing: Viết 5 câu trả lời mẫu TIẾNG ANH — mỗi câu là 1 từ/cụm từ ngắn, đúng và intelligible với từng câu hỏi trong đề, đạt điểm 3.`;
    ...
    if (/component\s*2|part\s*2/i.test(titleHint)) {
...
suggested_writing: Viết đoạn văn mẫu TIẾNG ANH 20–30 từ đúng yêu cầu đề, on topic, dùng simple connectors, đạt điểm 4–5.`;
    ...
    if (/component\s*3|part\s*3/i.test(titleHint)) {
